  ...
  for (const { rule, part } of ownBoxEntries()) {
    ...
    for (const decl of guarded) {
      ...
      if (decl.prop === 'height' || decl.prop === 'max-height') {
        ...
        // modelled band by the rail clearance test), `.layout-focus`
        // (proven inapplicable at <=720px by the mobile-mode test), the
        // mobile credit's own max-height, plus synchronous measurement,
        // which is removed before any paint.
        ...
        if (!railOwn && !railFocus && !mobileCredit && !railMeasurement) complaints.push(`${decl.prop}: ${decl.value} on "${part}"`);
      }
